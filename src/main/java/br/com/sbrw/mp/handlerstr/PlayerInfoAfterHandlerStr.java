...
public class PlayerInfoAfterHandlerStr extends ChannelInboundHandlerAdapter {
	...
	public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
		...
		if (isPlayerInfoPacket(buf)) {
			...
			if (mpTalker != null) {
				...
				mpTalker.setPlayerInfoAfterOk(true);
				playerInfoAfterOk(mpTalker);
			}
		}
		super.channelRead(ctx, msg);
	}

	private void playerInfoAfterOk(MpTalker mpTalker) {
		MpSession mpSession = MpSessions.get(mpTalker);
		if (mpSession.isAllPlayerInfoAfterOk()) {
	...
	private boolean isPlayerInfoPacket(ByteBuf buf) {
		return buf.getByte(0) == (byte) 0x34 //
